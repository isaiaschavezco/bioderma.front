  ...
  >
    ...
    <a-form class="form-question__form" :form="questionForm" @submit="onSubmitQuestion">
      ...
      <a-row style="margin-left: 4rem; margin-right: 4rem; margin-top:1rem;">
        <a-col class="text-center form-question__columnLabel" offset="2" :span="11">COLUMNA 1</a-col>
        <a-col class="text-center form-question__columnLabel" :span="11">COLUMNA 2</a-col>
      </a-row>
      ...
      for (let i = 0; i < 5; ++i) {
        if (i < 2) newAvailableValues[i] = true;
          ...
          for (let j = 0; j < i && available; ++j)
            available &=
              this.reactivesValues[j].length > 0 &&
              this.responsesValues[j].length > 0;
    ...
    onSubmitQuestion(e) {
      ...
      this.questionForm.validateFields(async (err, values) => {
        if (!err) {
          for (let i = 0; i < 5; ++i) {
            if (
              this.reactivesValues[i].length === 0 ||
              this.responsesValues[i].length === 0
            ) {
              for (let j = i; j < 5; ++j) {
          ...
          let questions = this.reactivesValues.filter(
            (val, index) => val.length > 0 && this.isAvailableOption[index]
          );
          let responses = this.responsesValues.filter(
            (val, index) => val.length > 0 && this.isAvailableOption[index]
          );
