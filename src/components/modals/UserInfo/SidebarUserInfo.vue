<template>
  <a-col id="sidebar-info" span="8">
    ...
    />
    <a-avatar
      style="color: white; backgroundColor: #1890ff; font-size:5rem;"
      :size="150"
      v-if="!(user.image && user.image !== 'URL')"
    >{{ `${user.name[0]}${user.lastName[0]}` }}</a-avatar>

    <div id="sidebar-general-info">
      <div id="sidebar-name">{{ user.name }}, {{ user.lastName }}</div>
      ...
      <div class="sidebar-info">
        ...
        <span class="data-info">{{ user.gender ? "Mujer" : "Hombre" }}</span>
      </div>
